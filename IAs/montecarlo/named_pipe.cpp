...
void Pipe::Write(const std::string& text)
{
    ...
    if(write(fd, text_array_, 2) < 0) std::cerr << "write failed" << std::endl;
...
void Pipe::Read(std::string& text)
{
    ...
    int reste = BOARD_SIZE * BOARD_SIZE + 2;
    ...
    {
        if(read(fd, text_array_, reste > TEXT_ARRAY_LENGTH ? TEXT_ARRAY_LENGTH: reste) < 0) std::cerr << "read failed" << std::endl;
